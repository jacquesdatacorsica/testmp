...
```
La base globale (marchés de mars 2015 jusqu'à nos jours) peut être récupérée également directement à partir des données préalablement téléchargées :
```{r restauration, echo=TRUE, cache=TRUE}
...
table2<-BOAMP[100000:100150,c(1,12:23)]
  ...
  datatable(extensions = 'Buttons',
            options = list(dom = 'Blfrtip',
                           ...
                           lengthMenu = list(c(10,25,50,-1),
                                             c(10,25,50,"All"))),rownames = FALSE)
```
```{r table_illustration 3, echo=TRUE, cache=TRUE}
table2<-BOAMP[100000:100150,c(1,24:35)]
...
table(str_sub(BOAMP$suppl_codenuts,1,2), str_sub(BOAMP$adjudicateurnuts,1,2))[,7]
```


Sur la période 2015-2021, la base permet d'identifier `r length(BOAMP$id)` contrats entre acheteurs publics et fournisseurs.
Parmis eux, `r round(mean(BOAMP$envir, rm.na=TRUE)*100,2)` % intègrent des clauses environnementales, contre `r round(mean(BOAMP$sociaux, rm.na=TRUE)*100,2)` %  de clauses sociales.

```{r results2, echo=TRUE}
mydata<-aggregate(BOAMP[which(str_length(BOAMP$adjudicateurnuts)==5),33], by=list(BOAMP[which(str_length(BOAMP$adjudicateurnuts)==5),]$adjudicateurnuts), FUN=mean)
...
On observe ainsi en 2020 `r NROW(BOAMP[which(str_sub(BOAMP$suppl_codenuts,1,2)!="FR"),]) ` marchés attribués à une entreprise hors du territoire national, pour un total de `r NROW(BOAMP)` contrats.
...
# Quelques séries chronologiques

où l'on remarque que les données les plus anciennes et les plus récentes méritent d'être consolid&es...
## évolution de l'usage des marchés clausés
```{r, echo=TRUE}
library(ggplot2)
